...
export const AddFavoriteModal = ({ onClose }) => {
    const { addFavorite, loading, error, successMessage, clearMessages } =
        useFavoritesStore();

    const [form, setForm] = useState({ alias: "", accountNumber: "" });
    ...
    const handleSubmit = async () => {
        ...
        const result = await addFavorite(form);
        if (result.success) {
    ...
    return (
        ...
        <div className="bg-white rounded-2xl shadow-xl w-full max-w-md mx-4 p-6">
            ...
            <div>
                <h2 className="text-base font-bold text-gray-900">Agregar favorito</h2>
                <p className="text-xs text-gray-400 mt-0.5">
                Guarda una cuenta para transferir rápido
                </p>
            </div>
            <button
                onClick={onClose}
            ...
            <div>
                ...
                className="mt-1 w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm font-mono text-gray-800 focus:outline-none focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100 transition"
                />
            </div>
            </div>
            ...
            <button
                ...
                disabled={loading || !form.alias.trim() || !form.accountNumber.trim()}
            ...
            >
                {loading ? "Guardando…" : "Guardar favorito"}
            </button>
            </div>
        </div>
        </div>
