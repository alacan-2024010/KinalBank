...
const TransactionCard = ({ tx, myAccountIds = [] }) => {
    ...
    const currency =
        CURRENCY_SYMBOLS[isCredit ? tx.currencyTo : tx.currencyFrom] ?? "Q";

    // Transferencia entrante → mostrar como depósito (verde)
    const displayType =
        tx.type === "TRANSFERENCIA" && isCredit ? "DEPOSITO" : tx.type;
        ...
        <div className="group relative overflow-hidden rounded-3xl border border-white/40 bg-white/70 backdrop-blur-xl p-5 shadow-sm hover:shadow-2xl hover:-translate-y-1 transition-all duration-300">
            ...
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <div className="flex items-center gap-2 mt-1">
                            <span className={`px-2 py-1 rounded-full text-[11px] font-semibold ${colors.bg} ${colors.text}`}>
                                {meta.label}
                            </span>
                ...
                <div className="text-right">
                    <p className={`text-xl font-black tabular-nums ${colors.amount}`}>
                        {isCredit ? "+" : "-"}
                        {currency}
                        {amount.toLocaleString("es-GT", { minimumFractionDigits: 2 })}
                    </p>
...
const StatCard = ({ title, value, subtitle, icon, gradient, text }) => (
...
export const ClientTransactionsPage = () => {
    ...
    const totalCreditos = transactions
        ...
        .reduce((sum, tx) => sum + Number(tx.amountReceived ?? 0), 0);
    ...
    const totalDebitos = transactions
        ...
        .reduce((sum, tx) => sum + Number(tx.amountSent ?? 0), 0);
        ...
        <div className="relative min-h-screen p-6">
            ...
            <div className="max-w-6xl mx-auto space-y-8">
                ...
                <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4">
                    ...
                    <div className="bg-white/70 backdrop-blur-xl border border-white/40 px-5 py-3 rounded-2xl shadow-sm">
                        ...
                        <p className="text-2xl font-black text-slate-800">
                            {pagination.totalRecords ?? transactions.length}
                        </p>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
                    ...
                    <StatCard
                        title="Entradas"
                        value={`Q ${totalCreditos.toLocaleString("es-GT", { minimumFractionDigits: 2 })}`}
                        subtitle="Depósitos, créditos y transferencias recibidas"
                    ...
                    <StatCard
                        title="Salidas"
                        value={`Q ${totalDebitos.toLocaleString("es-GT", { minimumFractionDigits: 2 })}`}
                        subtitle="Compras y transferencias enviadas"
                ...
                <div className="space-y-4">
                    {loading ? (
                        <div className="py-24 flex flex-col items-center">
                            ...
                            <p className="mt-4 text-slate-400">Cargando movimientos...</p>
                        </div>
                    ) : filtered.length === 0 ? (
                        <div className="bg-white/70 backdrop-blur-xl rounded-3xl border border-white/40 p-16 text-center">
                            ...
                            <h3 className="mt-5 text-xl font-bold text-slate-700">Sin movimientos</h3>
                            <p className="text-slate-400 mt-2">Tus transacciones aparecerán aquí</p>
                        </div>
                    ) : (
